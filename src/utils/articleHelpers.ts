...
export const getAuthorName = (article: Article) => {
...
export const getAuthorInfo = (article: Article) => {
  const { author } = article;
...
export const generateCitation = (
  ...
  const authors = [
    article.author,
    ...article.coAuthors.map((ca: any) => ca.author),
  ];
  const authorsText = authors
    .map((author: any) => `${author.lastName}, ${author.firstName.charAt(0)}.`)
    .join(", ");
  ...
  switch (format) {
    case "apa":
      return `${authorsText} (${year}). ${article.title}. ${
        article.journal?.name || "Revue Scientifique"
      }.`;

    case "mla":
      return `${authorsText} "${article.title}." ${
        article.journal?.name || "Revue Scientifique"
      }, ${year}.`;
