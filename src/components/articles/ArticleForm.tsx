...
interface ArticleFormProps {
  ...
  onSubmit: (data: ArticleFormData, isDraft: boolean) => Promise<{ success: boolean; message?: string; errors?: any }>
  isEditing?: boolean
}
...
export default function ArticleForm({ initialData, onSubmit, isEditing = false }: ArticleFormProps) {
  ...
  const steps = [
    { id: 1, name: 'Informations générales', description: 'Titre, résumé et contenu' },
    { id: 2, name: 'Détails et collaborateurs', description: 'Catégorie, mots-clés et co-auteurs' },
    { id: 3, name: 'Soumission', description: 'Déclarations et lettre d\'accompagnement' }
  ]

  const isStepValid = (step: number) => {
    switch (step) {
      case 1:
        return formData.title.length >= 10 && formData.abstract.length >= 100 && formData.content.length >= 1000
      case 2:
        return formData.categoryId && formData.keywords.trim().length > 0
      case 3:
        return formData.ethicsStatement.length >= 50 && formData.conflictOfInterest.length >= 20
      default:
        return false
    }
  }
  ...
  return (
    <div className="space-y-6">
                  ...
                  <div className="relative flex h-8 w-8 items-center justify-center">
                    {step.id < currentStep ? (
                      ...
                      <div className="h-8 w-8 rounded-full border-2 border-blue-600 bg-white flex items-center justify-center">
                        <span className="text-blue-600 text-sm font-semibold">{step.id}</span>
                      </div>
                    ) : (
      ...
      <div className="bg-white border rounded-lg p-6">
        ...
        {currentStep === 1 && (
          <div className="space-y-6">
            ...
            <div>
              ...
              />
              {errors.abstract && (
                <p className="mt-1 text-sm text-red-600">{errors.abstract[0]}</p>
              )}
              <p className="mt-1 text-sm text-gray-500">
        ...
        {currentStep === 2 && (
          <div className="space-y-6">
            <h2 className="text-lg font-semibold text-gray-900">Détails et collaborateurs</h2>
            ...
            <div>
              <label htmlFor="coAuthorEmails" className="block text-sm font-medium text-gray-700 mb-2">
              ...
              <input
                ...
                onChange={(e) => handleInputChange('coAuthorEmails', e.target.value)}
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                placeholder="email1@example.com, email2@example.com"
              />
              <p className="mt-1 text-sm text-gray-500">
                Emails des co-auteurs séparés par des virgules. Ils doivent avoir un compte sur la plateforme.
              </p>
            </div>
          </div>
        )}
        ...
        {currentStep === 3 && (
          <div className="space-y-6">
            <h2 className="text-lg font-semibold text-gray-900">Déclarations et soumission</h2>
            ...
            <div>
              ...
              {errors.ethicsStatement && (
                <p className="mt-1 text-sm text-red-600">{errors.ethicsStatement[0]}</p>
              )}
              <p className="mt-1 text-sm text-gray-500">
            ...
            <div>
              ...
              {errors.conflictOfInterest && (
                <p className="mt-1 text-sm text-red-600">{errors.conflictOfInterest[0]}</p>
              )}
              <p className="mt-1 text-sm text-gray-500">
            ...
            <div>
              ...
              />
              {errors.coverLetter && (
                <p className="mt-1 text-sm text-red-600">{errors.coverLetter[0]}</p>
              )}
              <p className="mt-1 text-sm text-gray-500">
        ...
        <div className="flex justify-between pt-6 border-t">
          ...
          <div className="flex space-x-3">
            ...
            {currentStep < 3 ? (
              ...
              >
                {loading ? 'Soumission...' : isEditing ? 'Mettre à jour' : 'Soumettre pour publication'}
              </button>
            )}
          </div>
        </div>
